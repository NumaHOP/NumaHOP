...
public class EsDocUnit {
	...
	@CompletionField(contexts = {
			@CompletionContext(name = EsConstant.SUGGEST_CTX_LIBRARY, type = ContextMappingType.CATEGORY) })
	...
	/**
	 * Liste des propriétés de l'unité documentaire regroupés dans une notice (DC, DCq,
	 * Custom)
	 */
	...
	public static EsDocUnit from(final DocUnit docUnit) {
		...
		esDocUnit.setPhysicalDocuments(
				docUnit.getPhysicalDocuments().stream().map(EsPhysicalDocument::from).collect(Collectors.toSet()));
		...
		if (docUnit.getWorkflow() != null) {
			final List<WorkflowStateKey> workflowStateKeys = docUnit.getWorkflow()
				.getCurrentStates()
				.stream()
				.map(DocUnitState::getKey)
				.collect(Collectors.toList());
		...
		docUnit.getDigitalDocuments()
			...
			.ifPresent(dlv -> {
				...
				final long totalLength = dlv.getDeliveries()
					.stream()
					.map(DeliveredDocument::getTotalLength)
		...
		esDocUnit
			.setRecords(docUnit.getRecords().stream().map(EsBibliographicRecord::from).collect(Collectors.toSet()));
		esDocUnit
			.setCinesReports(docUnit.getCinesReports().stream().map(EsCinesReport::from).collect(Collectors.toList()));
		esDocUnit.setIaReports(
				docUnit.getIaReports().stream().map(EsInternetArchiveReport::from).collect(Collectors.toList()));
