    ...
    function WorkflowGroupEditCtrl(
        ...
        function loadOptions() {
            ...
            if (angular.isDefined(currentUser)) {
                currentUser.then(function (result) {
                    if (result.library) {
                        $q.all([NumaHopInitializationSrvc.loadLibraries(), NumaHopInitializationSrvc.loadUsersForLibrary(result.library)]).then(function (data) {
        ...
        function onChangeLibrary(library) {
            $q.all([NumaHopInitializationSrvc.loadUsersForLibrary(library.identifier)]).then(function (data) {
                    ...
                    entity.$save(
                        ...
                        function (value) {
                            ...
                            if (creation) {
                                ...
                                NumahopEditService.updateMiddleColumn($scope.group, ['name'], $scope.pagination.items, $scope.newEntities);
                            }
                        },
                        function (response) {
        ...
        function onSuccess(value) {
            ...
            if ((!$scope.sel2Users || $scope.sel2Users.length === 0) && $scope.group.library) {
                $q.all([NumaHopInitializationSrvc.loadUsersForLibrary($scope.group.library.identifier)]).then(function (data) {
                    $scope.sel2Users = data[0];
                });
            }
